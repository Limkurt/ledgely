...
export async function submitExpenseAction(prevState: any, formData: FormData) {
  ...
  try {
    ...
    return {
      success: false,
      errors: { form: [error?.message || 'Failed to submit expense to database. Please check configuration.'] },
    };
  }
}
  ...
  try {
    if (isNotionConfigured && notion) {
      ...
      try {
        ...
        // Fallback properties for logs in case of title naming variance
        try {
          await notion.pages.create({
            parent: { database_id: BUDGET_LOG_DB_ID },
            properties: {
...
export async function addBudgetAdjustmentAction(prevState: any, formData: FormData) {
  const rawData = {
    amount: formData.get('amount'),
  ...
  try {
    ...
    return {
      success: false,
      errors: { form: [error?.message || 'Failed to add budget adjustment.'] },
    };
  }
}
